...
export async function loadData() {
  const bust = "v=13";
  const [league, playersDoc, rankingsDoc, vegasDoc, lexicon, seedIntel, alphaDoc] = await Promise.all([
    ...
    fetch(`data/vegas-rankings.json?${bust}`).then((r) => r.json()).catch(() => ({ players: {} })),
  ...
  const players = playersDoc.players.map((p) => ({
    ...p,
    ...(rankingsDoc.players?.[p.id] || {}),
    ...(vegasDoc.players?.[p.id] || {}),
  }));
  return {
    ...
    dataMeta: {
      ...
      vegasSource: vegasDoc.source || null,
      vegasGeneratedAt: vegasDoc.generatedAt || null,
    },
  };
}
